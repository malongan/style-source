...
| # | 项目 | 说明 | 工作量 | 难度 | 预计效果 |
|---|------|------|--------|------|---------|
| **P0-1** | **CSS/JS 压缩（terser + cssnano）** | gallery.css 24KB→~16KB，gallery-runtime.js 43KB→~14KB，合计 67KB→~30KB ⬇️ 55% | 小 | ★☆☆ | 加载速度翻倍 |
| **P0-2** | **渐进式图片加载（blur-up）** | 缩略图模糊占位 → 全尺寸渐入，当前 168 张缩略图已就绪，只需 JS 改造 | 中 | ★★☆ | 视觉体验提升 |
| **P0-3** | **标签去重** | 合并 `Flat`/`flat`、`Naive`/`naive`、`Q版`/`q版` 等大小写不一致标签，清理 null 标签 | 中 | ★★☆ | 数据整洁 |
| **P0-4** | **清理旧版 styles/gallery.html** | 删除 `styles/gallery.html` 旧版残留 | 小 | ★☆☆ | 避免混淆 |
